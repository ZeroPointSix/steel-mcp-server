...
class SteelSessionManager {
    ...
    constructor(
        ...
        if (typeof process.env.STEEL_LOCAL === 'undefined') {
            throw new Error("STEEL_LOCAL environment variable is not defined. Must be 'true' or 'false'.");
        }
...
// -----------------------------------------------------------------------------
const sessionManager = new SteelSessionManager(steelLocal, steelKey, globalWaitSeconds);
...
const TOOLS: Tool[] = [
    ...
    {
        name: 'click',
        description: 'Click an element on the page specified by its numbered label from the annotated screenshot',
        inputSchema: {
            type: 'object',
            properties: {
                label: {
    ...
    {
        name: 'scroll_down',
        description: 'Scroll down the page by a pixel amount - if no pixels are specified, scrolls down one page',
        inputSchema: {
            type: 'object',
            properties: {
                pixels: {
    ...
    {
        name: 'scroll_up',
        description: 'Scroll up the page by a pixel amount - if no pixels are specified, scrolls up one page',
        inputSchema: {
            type: 'object',
            properties: {
                pixels: {
...
async function handleNavigate(page: Page, args: any): Promise<CallToolResult> {
    ...
    if (!url) {
        return {
            isError: true,
            content: [{ type: 'text', text: 'URL parameter is required for navigation' }],
        };
    }
...
async function handleSearch(page: Page, args: any): Promise<CallToolResult> {
    ...
    if (!query) {
        return {
            isError: true,
            content: [{ type: 'text', text: 'Query parameter is required for search' }],
        };
    }
...
async function handleClick(page: Page, args: any): Promise<CallToolResult> {
    ...
    try {
        ...
        type ClickResult = { hasTargetBlank: true; href: string } | { hasTargetBlank: false };
        ...
        return {
            ...
            content: [
                {
                    type: 'text',
                    text: `Could not find clickable element with label ${label}. Error: ${(e as Error).message}`,
                },
            ],
        };
    }
 ...
 */
async function handleScrollDown(page: Page, args: any): Promise<CallToolResult> {
    const { pixels } = args;
    if (pixels !== undefined) {
    ...
    return {
        isError: false,
        content: [{ type: 'text', text: `Scrolled down by ${pixels ?? 'one page'}` }],
    };
}
 ...
 */
async function handleSaveUnmarkedScreenshot(page: Page, args: any): Promise<CallToolResult> {
...
async function runServer() {
    console.error(JSON.stringify({ message: 'Starting Web Voyager MCP server...' }));
    const transport = new StdioServerTransport();

    try {
        console.error(JSON.stringify({ message: 'Attempting to connect transport...' }));
...
process.on('SIGTERM', async () => {
    console.error(JSON.stringify({ message: 'Received SIGTERM, cleaning up...' }));
    await sessionManager.cleanup();
    process.exit(0);
});
