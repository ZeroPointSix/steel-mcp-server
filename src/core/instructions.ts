...
Use steel_batch for the next few known reversible checkout steps only when later targets need no fresh read. At a detected login/challenge boundary, hand off on the same session and resume only unrun steps. Stop before payment or final confirmation even when no detector fires.
